...
describe('replayWalletBalancesFromLedger', () => {
  ...
  it('reverses checkout when an order is cancelled', () => {
    const rows = [
      {
        amount: 100_000,
        status: 'SUCCESS',
      ...
      },
      {
        amount: 60_000,
        status: 'SUCCESS',
        note: 'Refund for cancelled order ORD-1 (Collateral released: 10000)',
      },
    ];
    const b = replayWalletBalancesFromLedger(rows);
    expect(b.availableBalance).toBe(100_000);
    expect(b.mainBalance).toBe(100_000);
